  ...
  const data = {
    ...
    questions: [
      ...
      },
      {
        ...
        items: [
          {
            original: [15, 18, 14, 20],
            answer: [14, 15, 18, 20],
          },
          {
            original: [33, 39, 35, 31],
            answer: [31, 33, 35, 39],
          },
          {
            original: [65, 74, 53, 81],
            answer: [53, 65, 74, 81],
          },
          {
            original: [57, 42, 36, 74],
            answer: [36, 42, 57, 74],
          },
          {
            original: [94, 99, 91, 96],
            answer: [91, 94, 96, 99],
          },
        ],
      },
      {
        ...
        items: [
          {
            original: [25, 16, 33, 12],
            answer: [33, 25, 16, 12],
          },
          {
            original: [44, 55, 33, 22],
            answer: [55, 44, 33, 22],
          },
          {
            original: [76, 79, 71, 78],
            answer: [79, 78, 76, 71],
          },
          {
            original: [85, 87, 81, 80],
            answer: [87, 85, 81, 80],
          },
          {
            original: [70, 10, 40, 60],
            answer: [70, 60, 40, 10],
          },
        ],
      },
      ...
      },
      {
        type: "even-odd",
        question:
          "Circle the objects in pairs and find out if the collections are even or odd. Tick the correct answer.",
        items: [
          {
            objects: "🐥🐥  🦜🦜  🧚🧚",
            count: 6,
  ...
  const getQuestionText = (sectionType, item) => {
    switch (sectionType) {
      case "fill-in-the-blank":
        return `${
          data.questions.find((_, i) => i === currentSectionIndex).question
        } ${item.prompt}`;
      case "comparison":
  ...
  const getFormattedAnswer = (sectionIndex, itemIndex, item) => {
    ...
    switch (sectionType) {
      ...
      case "place-value":
        ...
        const fullCorrect =
          String(tens).trim() === String(correctTens) &&
          String(ones).trim() === String(correctOnes);
  ...
  const renderQuestion = (item, index) => {
    switch (currentSection.type) {
      case "fill-in-the-blank":
        return (
          <div key={index} className="mb-6 flex items-center gap-4">
            <p className="text-lg mb-0">{item.prompt}</p>
            <input
              type="text"
              ...
              onChange={(e) =>
                handleAnswerChange(currentSectionIndex, index, e.target.value)
              }
            />
          </div>
        );
      ...
      case "choose-smallest":
        return (
          <div key={index} className="mb-6">
            <p className="text-lg mb-2">Options: {item.options.join(", ")}</p>
            <select
            ...
            <div className="flex gap-4">
              <div>
                ...
                <input
                  ...
                  value={
                    (
                      userAnswers[`${currentSectionIndex}-${index}`] || ""
                    ).split(",")[0] || ""
                  }
                  onChange={(e) => {
      ...
      case "even-odd":
        return (
          <div key={index} className="mb-6">
            <p className="text-5xl mb-2">{item.objects}</p>
            <select
              className="border border-gray-300 rounded px-3 py-2 w-full max-w-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={userAnswers[`${currentSectionIndex}-${index}`] || ""}
              onChange={(e) =>
          ...
          <div className="space-y-4">
            {results.map((result, i) => (
              ...
              >
                ...
                <p>
                  <strong>Your Answer:</strong> {result.user}
                </p>
                <p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-gray-500">
              Section {currentSectionIndex + 1} of {data.questions.length}
            </span>
          </div>
        ...
        <div className="mt-8 flex justify-end">
          ...
          >
            {currentSectionIndex === data.questions.length - 1
              ? "Finish"
              : "Next"}
          </button>
        </div>
